...
#' @export
Fit_DDModel <- function(model = NULL, rep = NULL, DL_model = NULL, grid_path = NULL, s_sampling = FALSE, trials = 10000, simplex_struc= c(20,2)){
  ...
  else if (Method == 1)
  {
    if (Output == 2)
    {
      ...
      {
        COMP_List[[i]] <-list(model,rep[[i]],s_sampling,trials,simplex_struc)
      }
    ...
    {
      COMP_List <- list(model,rep,s_sampling,trials,simplex_struc)
  ...
  else if (Method == 3)
  {
    if (Output == 2)
    {
      ...
      for (i in 1:length(rep))
      {
        INP <- t(DDRep_wide(rep[[i]]))
        PRE <- as.numeric(predict(DL_model,INP))
        COMP_List[[i]] <-list(model,rep[[i]],s_sampling,trials,simplex_struc,PRE)
      }
    ...
    {
      INP <- t(DDRep_wide(rep))
      PRE <- as.numeric(predict(DL_model,INP))
      COMP_List <- list(model,rep,s_sampling,trials,simplex_struc,PRE)
